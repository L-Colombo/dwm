...
void drawtab(Monitor *m) {
    ...
    if (tot_width > m->ww) { // not enough space to display the labels, they
    ...
    for (c = m->clients; c; c = c->next) {
        ...
        w = m->tab_widths[i];
        drw_setscheme(drw,
                      scheme[(c == m->sel) ? SchemeInfoSel : SchemeInfoNorm]);
...
void togglefollow(const Arg *arg) {
    selmon->wfsymbol[0] =
        (selmon->wfsymbol[0] == WFACTIVE) ? WFINACTIVE : WFACTIVE;
    drawbars();
}
